...
function trustedText(message: Message): string {
...
function currentTurn(messages: readonly Message[]): {
  ...
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    ...
    for (let tail = index + 1; tail < messages.length; tail += 1) {
      if (messages[tail]?.source.kind === 'tool') toolResults += 1
    }
...
function enumeratedRequirementCount(text: string): number {
...
export function classifyComplexity(
  options: Pick<GenerateOptions, 'messages' | 'sessionId' | 'provider' | 'model' | 'purpose'>,
  ...
  if (turn.message === undefined) return { tier: 'normal', score: 0, reasons: ['no-user-message'], turnKey }
  ...
  for (const keyword of config.additionalComplexKeywords) {
    if (folded.includes(keyword.toLocaleLowerCase())) customMatches += 1
  }
  if (customMatches > 0) {
...
export class EscalationCache {
  private readonly decisions = new Map<string, ComplexityDecision>()
